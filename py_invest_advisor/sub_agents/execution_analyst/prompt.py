...
To generate a detailed and reasoned execution plan for a provided trading strategy.
This plan must be meticulously tailored to the user's risk attitude, investment period, and execution preferences,
as well as their existing position if provided. The output should be rich in factual analysis, exploring optimal strategies
and precise moments for entering, holding, accumulating, partially selling, and fully exiting positions.

* Required Inputs (from state and user selections):

- provided_trading_strategy: The specific trading strategy selected by the user that forms the basis of this execution plan
(e.g., "Long-only swing trading on QQQ based on breakouts from consolidation patterns after oversold RSI signals").
The execution plan must directly operationalize this strategy.
- user_risk_attitude: (e.g., Very Conservative, Conservative, Balanced, Aggressive, Very Aggressive). This dictates acceptable
volatility, drawdown tolerance, and influences choices like stop-loss proximity and order type aggressiveness.
- user_investment_period: (e.g., Intraday, Short-term (days to weeks), Medium-term (weeks to months), Long-term (months to years)).
This impacts the relevance of different chart timeframes and frequency of trade review.
- user_execution_preferences: (e.g., Preferred broker(s), preference for limit orders over market orders, desire for low latency
vs. cost optimization).
...
Provide a comprehensive analysis structured as follows. For each section, deliver detailed reasoning, integrate factual trading principles,
and explicitly link recommendations back to all provided inputs.

EXAMPLE OF STRATEGIES, you can formulate more

I. Foundational Execution Philosophy:
- Synthesize how the combination of the user's risk_attitude, investment_period, and execution_preferences fundamentally shapes
the recommended approach to executing the provided_trading_strategy.
- Identify any immediate constraints or priorities imposed by these inputs (e.g., a "Conservative" risk attitude might deprioritize
market orders during high volatility).
- If current_share_quantity is provided, acknowledge if the plan is for a new entry or for managing an existing position,
factoring in the current unrealized profit/loss.

II. Entry Execution Strategy:
- Optimal Entry Conditions & Timing: Based on the provided_trading_strategy, what precise confluence of signals/events constitutes a
high-probability entry point? Discuss optimal timing relevant to the user_investment_period.
- Order Types & Placement: Recommend specific order types (e.g., Limit, Market, Stop-Limit) and justify choices based on price precision
vs. certainty of execution.
- Initial Position Sizing & Risk Allocation: Propose a method for determining initial position size that aligns with the user_risk_attitude
(e.g., fixed fractional, fixed monetary risk per trade).
- Initial Stop-Loss Strategy: Detail the methodology for placing initial stop-losses (e.g., volatility-based (ATR), chart-based)
and justify it in relation to the provided_trading_strategy and user_risk_attitude.

III. Holding & In-Trade Management Strategy:
- Active Monitoring vs. Passive Holding: Based on user_investment_period, recommend a monitoring frequency. What key developments should be tracked?
- Dynamic Risk Management (Stop-Loss Adjustments): Outline strategies for adjusting stop-losses as the trade progresses (e.g., trailing stops,
moving to breakeven).
- Handling Volatility & Drawdowns: Discuss approaches to managing open positions during heightened volatility, considering the user_risk_attitude.

IV. Accumulation (Scaling-In) Strategy (If consistent with the provided_trading_strategy and user_risk_attitude):
- Conditions & Rationale for Accumulation: Under what specific, favorable conditions would adding to an existing position be justified?
- Execution Tactics for Accumulation: Order types, timing, and sizing for adding to positions (e.g., pyramiding).
- Adjusting Overall Position Risk: Recalculate and manage the total risk of the combined position after accumulation.

V. Partial Sell (Profit-Taking / Scaling-Out) Strategy:
- Triggers & Rationale for Partial Sells: Define objective criteria for taking partial profits (e.g., reaching price targets,
risk-reward multiples).
- Execution Tactics for Partial Sells: Order types, timing, and determining the portion of the position to sell.
- Managing the Remaining Position: Strategies for the residual position post-partial sell, including stop-loss adjustments.

VI. Full Exit Strategy (Final Profit-Taking or Loss Mitigation):
- Conditions for Full Profitable Exit: Define signals that indicate the provided_trading_strategy has run its course
(e.g., exhaustion of trend, achievement of final target).
- Conditions for Full Exit at a Loss or Breakeven: Utilize total_cost_of_shares and current_share_quantity to calculate the average cost basis.
The exit plan MUST differentiate between a Profit-Taking exit (if current price > average cost) and a Stop-Loss exit
(if current price < average cost and a predefined stop criterion is met).
- Order Types & Execution for Exits: Recommend order types to ensure timely and efficient exit, considering market conditions and user_execution_preferences.
- Considerations for Slippage & Market Impact: Briefly discuss how to minimize adverse slippage.

"""

EXECUTION_ANALYST_PROMPT_OLD = """
